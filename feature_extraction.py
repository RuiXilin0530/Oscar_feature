...
DATA_ROOT = os.getenv('COCO_IMG_ROOT', '/storage_fast/xlrui/val2014/')
MIN_BOXES = 10
MAX_BOXES = 100

parser = argparse.ArgumentParser()
parser.add_argument('--split', default='train2014', help='train2014, val2014')
parser.add_argument('--batchsize', default=12, type=int, help='batch_size')
...
def doit(detector, raw_images):
    with torch.no_grad():
        ...
        # attr_probs = pred_attr_logits[..., :-1].softmax(-1)
        # print(attr_prob.shape)
        # max_attr_probs, max_attr_labels = attr_probs.max(-1)
        # print(max_attr_prob.shape)
        ...
        for probs, boxes, image_size in zip(probs_list, boxes_list, images.image_sizes):
            ...
            # raw_instances.attr_scores = max_attr_prob
            # raw_instances.attr_classes = max_attr_label
...
def dump_features(writer, detector, pathXid, val=None, object_val=None, line=None):
    ...
    for img, img_id, instances, features in zip(imgs, img_ids, instances_list, features_list):
        ...
        # print("Original image size (height and width): ", (raw_height, raw_width))
        ...
        # print("feature")
        # print(features.shape)
        a = instances.pred_boxes.tensor.numpy()
        # print("a")
        # print(a.shape)
        # print(a[0])
        a[:, (0, 1)] = a[:, (1, 0)]
        ...
        b = a[:, 2] - a[:, 0]
        c = a[:, 3] - a[:, 1]
        d = np.ones((a.shape[0],a.shape[1]+2))*np.nan
        d[:,:a.shape[1]] = a
        d[:,-2]=b
        d[:,-1]= c
        # print(d[0])
        f = np.ones((d.shape[0], 2054))*np.nan
        f[:, :2048] = features
        f[:, 2048:2054] = d
        # print(features[0])
        # print(f[0])
        val[img_id] = torch.from_numpy(f).float()
        a = ""
        for p in instances.pred_classes:
            a = a + line[p] + ' '
        # print(a)
        object_val[img_id] = a
        # val[img_id] = torch.FloatTensor(val[img_id])
        # print(val[img_id].dtype)
        # print("d")
        # print(d.shape)
        # print(d[0])
        ...
        # v.save("/storage/xlrui/heng/mmqa/img_result/" + img_id)
        ...
        # }
        # # print(base64.b64decode(item["img_id"]))
        # # print(base64.b64decode(item["attrs_id"]))
        # # print(base64.b64decode(item['features']))
        # # print(base64.b64decode(item["boxes"]))

        # writer.writerow(item)
    return val, object_val
...
def extract_feat(outfile, detector, pathXid):
    ...
    with open(outfile, 'a') as tsvfile:
        val = {}
        object_val = {}
        f = open("./data/genome/1600-400-20/objects_vocab.txt")
        line = f.readlines()
        ...
        for start in tqdm.tqdm(range(0, len(pathXid), args.batchsize)):
            ...
            val, object_val = dump_features(writer, detector, pathXid_trunk, val, object_val, line)
            # print(val.keys())
            # print(len(val))
            """
            try:
        ...
        torch.save(object_val, 'object.pt')
        

def load_image_ids(img_root, split_dir, needid = None):
...
def build_model():
    ...
    if args.weight == 'mask':
        ...
        cfg.MODEL.WEIGHTS = "https://dl.fbaipublicfiles.com/detectron2/COCO-InstanceSegmentation/mask_rcnn_R_101_C4_3x/138363239/model_final_a2914c.pkl"
...
if __name__ == "__main__":
    a = torch.load("/storage_fast/xlrui/vqa/2k/val_img_frcnn_feats.pt")
    pathXid = load_image_ids(DATA_ROOT, args.split, a)     # Get paths and ids
    # a = "COCO_val2014_000000000241"
    # pathXid = [("/storage_fast/xlrui/val2014/" + a + ".jpg", a)]
    detector = build_model()
    extract_feat('/storage/xlrui/heng/mmqa/%s_d2obj36_batch.tsv' % args.split, detector, pathXid)
